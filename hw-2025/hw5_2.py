import score_tools
# 功能性（Laplace表面编辑方法正确性以及实时性）：
# - 局部区域的选择与编辑（bonus，比较困难）
# - Laplace表面编辑方法正确性（2）
#   - 老师上课介绍的简单的方法 (2)
#   - 论文中的方法 (bonus)
# - 实时拖动区域显示结果（1）
# - 不同权重的Laplacian的选取，至少实现均匀权重和余切权重（2）
#   - 均匀权重（1）
#   - 余切权重（1）

function_group = score_tools.AddGroup("功能", subgroups=[
        score_tools.AddGroup("Basics",maximum=5,minimum=0, basic_score=0,subgroups=[
            score_tools.FixedLeafGroup("Laplace表面编辑方法正确性", 2, True),
            score_tools.FixedLeafGroup("实时拖动区域显示结果", 1, True),
            score_tools.AddGroup(
                "不同权重的Laplacian的选取", maximum=2, minimum=0, basic_score=0, subgroups=[
                    score_tools.FixedLeafGroup("均匀权重", 1, True),
                    score_tools.FixedLeafGroup("余切权重", 1, True),
                ]
            )
        ]),
        score_tools.AddGroup("Bonus", maximum=1e9, minimum=0, basic_score=0, subgroups=[
            score_tools.MutableLeafGroup("使用论文中的方法", score_max=0.25, score_min=0, initial=0),
            score_tools.MutableLeafGroup("局部区域的选择与编辑", score_max=0.5, score_min=0, initial=0)
        ])
...
# - 程序效率不高（-0~-2）
#   - 没有使用预处理或者预处理但不完全，比如预处理之后又带着计算（-2）
# - 其他不规范写法（酌情）
# - 程序发生崩溃（不是由框架引起的 -2）
# - 冗余操作（酌情）
#   - 连节点的时候连接了一些多余的节点（酌情）
...
prog_group = score_tools.AddGroup("程序", maximum=5.5,minimum=0,basic_score=0,subgroups=[
    score_tools.AddGroup("Basics", maximum=5, minimum=0, basic_score=5, subgroups=[
        score_tools.AddGroup(
            ...
            subgroups=[
                score_tools.FixedLeafGroup("没有使用预处理或者预处理但不完全，比如预处理之后又带着计算", -2,False),
            ]
        ),
