...
function_group = score_tools.AddGroup("功能性（质点弹簧系统实现正确性）", subgroups=[
        score_tools.AddGroup("Basics(WCSPH)",maximum=5,minimum=0, basic_score=0,subgroups=[
            score_tools.FixedLeafGroup("正确安排 step() 步骤", 1, True),
            score_tools.FixedLeafGroup("compute_density() 密度和压强计算", 2, True),
            score_tools.FixedLeafGroup("non-pressure accelerations 计算", 1, True),
            score_tools.FixedLeafGroup("pressure gradient acceleration 计算", 1, True),
        ]),
        score_tools.AddGroup("Bonus", maximum=1e9, minimum=0, basic_score=0, subgroups=[
            score_tools.MutableLeafGroup("实现IISPH", score_max=0.25, score_min=0, initial=0),
...
report_group = score_tools.AddGroup("报告",maximum=5.5,minimum=0, basic_score=0, subgroups=[
    ...
    score_tools.AddGroup("Bonus", maximum=1e9, minimum=0, basic_score=0, subgroups=[
            ...
            score_tools.MutableLeafGroup("重建表面并渲染结果", 0.25,0,0),
        ])
])
