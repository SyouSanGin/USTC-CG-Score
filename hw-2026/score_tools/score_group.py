...
class MutableLeafGroup(LeafGroup):
    ...
    def get_score_detail(self) -> str:
        return f"Range: {self.score_min}~{self.score_max}"
...
def generate_report(tree: ScoreGroup, addon:str):
    ...
    def _inner_generate_report(t: ScoreGroup, level:int):
        ...
        if isinstance(t, LeafGroup):
            reportstr += " - " + t.get_score_detail()
        reportstr += '\n'
        res += reportstr
