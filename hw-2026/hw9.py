...
function_group = score_tools.AddGroup("功能", subgroups=[
	score_tools.AddGroup("Basics", maximum=4.25, minimum=0, basic_score=0, subgroups=[
		score_tools.FixedLeafGroup("实现半隐式质点弹簧仿真（稳定性与能量表现合理）", 2.25, True),
		score_tools.MutableLeafGroup("在不同参数下进行仿真并对比结果（可视化与定性分析）", score_max=2.0, score_min=0, initial=2.),
	]),


	score_tools.AddGroup("Bonus", maximum=1e9, minimum=0, basic_score=0, subgroups=[
		score_tools.MutableLeafGroup("基于惩罚力的布料与球之间的碰撞处理", score_max=0.5, score_min=0, initial=0),
		score_tools.MutableLeafGroup("Local-Global思想的质点弹簧系统加速算法", score_max=0.5, score_min=0, initial=0),
		score_tools.MutableLeafGroup("体网格的质点弹簧仿真", score_max=0.5, score_min=0, initial=0),
	]),
...
extension_group = score_tools.AddGroup("拓展任务", maximum=5.5, minimum=0, basic_score=0, subgroups=[
    ...
    score_tools.MutableLeafGroup("并行化加速", score_max=0.3, score_min=0, initial=0),
    score_tools.MutableLeafGroup("用深度学习方法进行模拟，并进行定性定量分析", score_max=0.7, score_min=0, initial=0),
