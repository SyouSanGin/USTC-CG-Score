...
function_group = score_tools.AddGroup("功能", subgroups=[
        score_tools.AddGroup("Basics",maximum=4.25,minimum=0, basic_score=0,subgroups=[
            ...
            score_tools.AddGroup(
                "形区域的融合", maximum=4, minimum=0, basic_score=0, subgroups=[
                    ...
                    score_tools.FixedLeafGroup("调用正确的求解器", 0.75, True),
                    score_tools.FixedLeafGroup("得到正确结果", 0.5, True),
                ]
            )
        ]),
        score_tools.AddGroup("Bonus", maximum=1e9, minimum=0, basic_score=0, subgroups=[
            ...
            score_tools.MutableLeafGroup("其他创新",score_max=0.2,score_min=0,initial=0),
        ])
...
prog_group = score_tools.AddGroup("程序", maximum=5.5,minimum=0,basic_score=0,subgroups=[
    score_tools.AddGroup("Basics", maximum=4.25, minimum=0, basic_score=4.25, subgroups=[
        score_tools.FixedLeafGroup("没有处理区域移动到背景图像之外的情况", score=-1, status=False),
        score_tools.AddGroup(
            '程序效率不高',maximum=0, minimum=-2,basic_score=0,
            subgroups=[
                score_tools.FixedLeafGroup("没有使用稀疏矩阵", -1,False),
                score_tools.MutableLeafGroup("其他影响效率的冗余操作", score_max=0, score_min=-1, initial=0),
                score_tools.MercifulLeafGroup("其他影响效率的冗余操作(不作扣分)")
            ]
        ),
        score_tools.MutableLeafGroup("其他不规范写法", score_max=0, score_min=-0.5, initial=0),
