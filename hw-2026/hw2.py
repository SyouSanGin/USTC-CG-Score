...
# 功能(5.5)
#     Basics(4.25)
#         至少实现了半残的IDW/RBF(如只能实现仿射变换等不完全功能)(0.25)
#         实现IDW(2)
#         实现RBF(2)
...
function_group = score_tools.AddGroup("功能", subgroups=[
        score_tools.AddGroup("Basics",maximum=4.25,minimum=0, basic_score=0,subgroups=[
            score_tools.FixedLeafGroup("至少实现了半残的IDW/RBF(如只能实现仿射变换等不完全功能)", 0.25, True),
...
prog_group = score_tools.AddGroup("程序", maximum=5.5,minimum=0,basic_score=0,subgroups=[
    score_tools.AddGroup("Basics", maximum=4.25, minimum=0, basic_score=4.25, subgroups=[
        ...
        score_tools.MutableLeafGroup("没有处理点数较少的情况，存在鲁棒性问题",score_max=0, score_min=-1, initial=0),
        score_tools.FixedLeafGroup("存在内存泄漏，例如申请了 warper* 指针的内存，但是没有释放，推荐使用智能指针或者主动调用delete", -1, False),
